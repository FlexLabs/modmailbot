...
|ignoredMessageResponse|"There are no commands. If you would like to speak with staff, please ask a question here."|The response that gets sent when a message starts with something in the `ignoredWords` or `ignoredPrefixes` config options|
|genericResponse|"If you would like to speak with staff, please ask a question here."|The response that gets sent when `ignoreNonAlphaMessages` is true and the message doesn't contain any alphabetic characters, or their message character length is shorter than `minContentLength`|
|minContentLength|3|The minimum amount of characters required to send in a message to start a new thread. Set to `null` to disable this limit entirely|
